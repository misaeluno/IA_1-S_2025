...
class MiVentana(QMainWindow):
    ...
    def guardar_zoom_bn(self):
        ...
        if self.zoom_label.pixmap() is None or self.zoom_label.pixmap().isNull():
            QMessageBox.warning(self, "Error al guardar", "No hay imagen en el zoom para guardar.")
            return

        # Obtener el pixmap del zoom_label
        zoom_pixmap = self.zoom_label.pixmap()
        
        # Convertir QPixmap a QImage para poder manipular los píxeles
        zoom_image = zoom_pixmap.toImage()

        # Convertir la imagen a blanco y negro (escala de grises)
        # QImage.Format_Grayscale8 es un formato de 8 bits por píxel para escala de grises
        # Si la imagen original ya es de 8 bits, se puede usar convertToFormat directamente.
        # Si es de 32 bits (ARGB32), se puede convertir a 8 bits de escala de grises.
        if zoom_image.format() != QImage.Format_Grayscale8:
            zoom_image = zoom_image.convertToFormat(QImage.Format_Grayscale8)

        # Abrir diálogo para guardar el archivo
        file_path, _ = QFileDialog.getSaveFileName(
            self, "Guardar imagen de zoom en B/N", "imagen_zoom_bn.png",
            "Imágenes PNG (*.png);;Todos los archivos (*)"
        )

        if file_path:
            if zoom_image.save(file_path, "PNG"):
                QMessageBox.information(self, "Guardado exitoso", f"Imagen guardada en: {file_path}")
            else:
