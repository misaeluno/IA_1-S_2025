...
from datetime import datetime # Importar para generar marcas de tiempo
...
class MiVentana(QMainWindow):
    def __init__(self):
        ...
        self.cost_history = [] # Nuevo: Para almacenar el costo en cada epoch
        ...
        lambda_layout.addWidget(QLabel("Valor de Lambda ($\lambda$):"))
        ...
        # Control para el valor de Alpha (tasa de aprendizaje)
        alpha_layout = QHBoxLayout()
        alpha_layout.addWidget(QLabel("Valor de Alpha ($\alpha$):"))
        self.spinbox_alpha = QDoubleSpinBox()
        self.spinbox_alpha.setRange(0.001, 1.0) # Rango típico para tasas de aprendizaje
        self.spinbox_alpha.setSingleStep(0.001)
        self.spinbox_alpha.setValue(0.5) # Valor por defecto de alpha
        alpha_layout.addWidget(self.spinbox_alpha)
        layout_derecha.addLayout(alpha_layout)

        # Control para el valor de Epsilon (inicialización de pesos)
        epsilon_layout = QHBoxLayout()
        epsilon_layout.addWidget(QLabel("Valor de Epsilon ($\epsilon$):"))
        self.spinbox_epsilon = QDoubleSpinBox()
        self.spinbox_epsilon.setRange(0.001, 1.0) # Rango típico para epsilon
        self.spinbox_epsilon.setSingleStep(0.001)
        self.spinbox_epsilon.setValue(0.05) # Valor por defecto de epsilon
        epsilon_layout.addWidget(self.spinbox_epsilon)
        layout_derecha.addLayout(epsilon_layout)
    ...
    def realizar_comprobacion(self):
        ...
        if self.X_train is not None and self.X_train.shape[0] > 0:
            ...
            if (max_val_train - min_val_train) > 1e-8:
                input_vector_normalized = (input_vector - min_val_train) / (max_val_train - min_val_train)
            else:
                input_vector_normalized = np.zeros_like(input_vector)
            print(f"DEBUG: Vector de entrada normalizado al rango [{np.min(input_vector_normalized):.4f}, {np.max(input_vector_normalized):.4f}] para la predicción.")
        else:
            ...
            if (max_val_single - min_val_single) > 1e-8:
                input_vector_normalized = (input_vector - min_val_single) / (max_val_single - min_val_single)
            else:
                input_vector_normalized = np.zeros_like(input_vector)


        # 3. Realizar la predicción
        predicted_class_num = predict_single_image(input_vector_normalized, self.trained_weights, len(self.class_names))
        ...
        if predicted_class_num is not None:
            ...
            # --- GUARDAR IMAGEN Y DATOS AUTOMÁTICAMENTE ---
            script_dir = os.path.dirname(os.path.abspath(__file__))
            
            # Guardar la imagen de 15x15
            timestamp = datetime.now().strftime("%Y%m%d_%H%M%S")
            image_filename = f"comprobacion_img_{timestamp}.png"
            image_save_path = os.path.join(script_dir, image_filename)
            
            try:
                if qimage.save(image_save_path, "PNG"):
                    print(f"Imagen de comprobación guardada en: {image_save_path}")
                else:
                    print(f"Error: No se pudo guardar la imagen de comprobación en: {image_save_path}")
            except Exception as e:
                print(f"Error al guardar la imagen de comprobación: {e}")

            # Guardar los datos en comprobacion.txt
            data_filename = "comprobacion.txt"
            data_save_path = os.path.join(script_dir, data_filename)
            
            try:
                with open(data_save_path, "a") as f: # "a" para añadir al final del archivo
                    f.write(f"--- Comprobación {timestamp} ---\n")
                    f.write(f"Imagen: {image_filename}\n")
                    f.write(f"Clase Predicha: {class_name} (ID: {predicted_class_num})\n")
                    f.write(f"Vector de Entrada (FFT, Aplanado): {input_vector.tolist()}\n") # Guardar el vector original, no el normalizado
                    f.write(f"-----------------------------------\n\n")
                print(f"Datos de comprobación guardados en: {data_save_path}")
            except Exception as e:
                print(f"Error al guardar los datos de comprobación: {e}")
...
#---------------------------------------------------------------------------------
        ...
        alpha = self.spinbox_alpha.value() # Obtener el valor de alpha desde el spinbox
        epsilon = self.spinbox_epsilon.value() # Obtener el valor de epsilon desde el spinbox
        ...
        self.cost_history = [] # Limpiar el historial de costos antes de un nuevo entrenamiento
        ...
        for epoch in range(epochs):
            ...
            J = fit_result['J']
            self.cost_history.append(J) # Almacenar el costo para el gráfico
        ...
        print(f"Costo final del entrenamiento (J): {J:.6f}") 
        
        # Evaluar en el conjunto de pruebas después del entrenamiento (opcional, pero es una buena práctica)
        if self.X_test is not None and self.y_test is not None:
            self.evaluate_model()
        
        # Mostrar el gráfico de la función de costo
        self.plot_cost_function()

    def plot_cost_function(self):
        """
        Genera y muestra un gráfico de la función de costo a lo largo de las iteraciones.
        """
        if not self.cost_history:
            QMessageBox.warning(self, "Gráfico de Costo", "No hay historial de costos para graficar. Entrena el modelo primero.")
            return

        plt.figure(figsize=(10, 6))
        plt.plot(self.cost_history, label='Función de Costo (J)')
        plt.title('Convergencia de la Función de Costo durante el Entrenamiento')
        plt.xlabel('Epoch')
        plt.ylabel('Costo (J)')
        plt.grid(True)
        plt.legend()
    ...
    def mostrar_matriz_confusion(self):
        ...
        incluyendo Recall, Precisión y F-measure por clase y el promedio total.
        """
        if self.trained_weights is None:
        ...
        labels_sorted = sorted(self.class_names.keys())
        cm = confusion_matrix(y_true_nums.astype(int), y_pred_nums.astype(int), labels=labels_sorted)

        # Calcular Recall, Precision, F-measure para cada clase
        ...
        for i in range(num_classes):
            ...
            fmeasure_per_class.append(fmeasure)

        # Calcular promedios totales (macro average)
        avg_recall = np.mean(recall_per_class)
        avg_precision = np.mean(precision_per_class)
        avg_fmeasure = (2 * avg_recall * avg_precision) / (avg_recall + avg_precision) if (avg_recall + avg_precision) > 0 else 0


        # 4. Visualizar la matriz de confusión con métricas adicionales
        fig, ax = plt.subplots(figsize=(12, 8)) # Ajustar tamaño para acomodar las nuevas columnas
        
        # Ocultar ejes para dibujar la tabla
        ax.axis('off')
        ax.set_title("Matriz de Confusión y Métricas de Desempeño", fontsize=16)

        # Preparar datos para la tabla
        # Encabezados de columna: Clases + Precision, Recall, F-measure
        column_headers = [f"Clase {c}" for c in labels_sorted] + ["Precision", "Recall", "F-measure"]
        
        # Encabezados de fila: Clases + "Promedio Total"
        row_headers = [self.class_names.get(c, f"Clase {c}") for c in labels_sorted] + ["Promedio Total"]

        # Crear la tabla de datos
        table_data = []
        for i in range(num_classes):
            row = [f"{cm[i, j]}" for j in range(num_classes)] # Valores de la matriz de confusión
            row.append(f"{precision_per_class[i]:.2f}")
            row.append(f"{recall_per_class[i]:.2f}")
            row.append(f"{fmeasure_per_class[i]:.2f}")
            table_data.append(row)
        
        # Añadir la fila de promedios
        avg_row = [""] * num_classes # Celdas vacías para la parte de la matriz
        avg_row.append(f"{avg_precision:.2f}")
        avg_row.append(f"{avg_recall:.2f}")
        avg_row.append(f"{avg_fmeasure:.2f}")
        table_data.append(avg_row)

        # Crear la tabla de matplotlib
        ...
        table.scale(1.2, 1.2) # Ajustar el tamaño de las celdas

        # Estilizar la tabla para que se parezca a la imagen (bordes, colores)
        for (i, j), cell in table.get_celld().items():
            cell.set_edgecolor('black')
            cell.set_linewidth(1)
            if i == 0 or j == -1: # Encabezados de columna y fila
                cell.set_facecolor('#D3D3D3') # Gris claro
            if i > 0 and j >= num_classes: # Celdas de métricas (Precision, Recall, F-measure)
                cell.set_facecolor('#ADD8E6') # Azul claro
            if i == num_classes and j >= 0: # Fila de promedio total
                cell.set_facecolor('#90EE90') # Verde claro

        plt.tight_layout()
        plt.show()

# Función para calcular Recall, Precision, F-measure
def performance(p, y, class_label):
    """
    Calcula Recall (R), Precisión (P) y F-measure (Fm) para una clase específica.
    p: predicciones (array de etiquetas numéricas)
    y: etiquetas verdaderas (array de etiquetas numéricas)
    class_label: la etiqueta numérica de la clase para la cual calcular las métricas
    """
    TP = np.sum((p == class_label) & (y == class_label))
    FP = np.sum((p == class_label) & (y != class_label))
    FN = np.sum((p != class_label) & (y == class_label))
    
    # nP (Número de Positivos Reales) = TP + FN
    nP = TP + FN

    recall = TP / nP if nP > 0 else 0
    precision = TP / (TP + FP) if (TP + FP) > 0 else 0
    fmeasure = (2 * recall * precision) / (recall + precision) if (recall + precision) > 0 else 0
